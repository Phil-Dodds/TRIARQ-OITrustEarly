...
| CC-019 | **UAT Bug 1 follow-up — profile load timing race.** After CC-017 (showUserManagement getter fix), Phil's sidebar still missed the Admin item on routes other than `/home`. Root cause: `app.component.ts:47` called `profileService.loadProfile()` synchronously in `ngOnInit` guarded by `auth.isAuthenticated()`. AuthService's session restore via `getSession()` is async and completes AFTER ngOnInit. On deep-link landing routes (`/delivery/cycles`, etc.), `isAuthenticated()` returned false at ngOnInit time → loadProfile never called → `profile$` stayed null → sidebar's role filter excluded every role-restricted item, including Admin. Profile only loaded if Phil happened to land on `/home` first (HomeComponent also calls loadProfile). Fix: wrap the loadProfile call in `auth.waitForInit().then(...)` so the auth state is established before the role check. |
| CC-020 | **UAT Bug 2 follow-up — Edit form Division dropdown re-patch.** After CC-018 (get_delivery_cycle division_name enrichment), the detail panel rendered Division correctly but the Edit form's Division dropdown remained blank. Root cause: the form was initialised in `ngOnInit` with `cycle.division_id` BEFORE `availableDivisions` populated (the latter comes from an async `get_user_divisions` MCP call in `loadDivisions`). With reactive forms + a `<select>` bound via `*ngFor`, Angular's `NgSelectControlValueAccessor` does not reliably re-sync the DOM value once options appear after the form value was set. Fix: re-patch the form's `division_id` value inside the `loadDivisions` MCP callback, after `availableDivisions` is set and before `cdr.markForCheck()`. The re-patch uses `setValue(currentDivId, { emitEvent: false })` so onValueChanges subscribers don't refire spuriously. Same pattern applies to any other dropdown bound to dynamic options — flagged as a pattern note for future contracts. |

**Gap check:** CC-001 through CC-020 — sequential, no gaps. PASS per Rule 17.
