// gate-approvals-tab.component.ts — Pathways OI Trust
// Contract 30 / D-472 (WS1.3). Tab 1 of My Actions — Accountable items
// (list_pending_approvals where item_type = 'accountable').
//
// Filter pattern replicates S-012 (active chips) / S-013 (drill-in panel) /
// S-011 (Apply/Clear commit) locally — there is no shared filter-panel component
// to reuse (CC-30; extraction flagged as a next-contract candidate). Sort per
// S-036. Default 21-day filter on created_at; clearable. Filter/sort memory
// persists per D-171 via ScreenStateService (screen key actions.gate-approvals).
//
// Approve and the Initiative chip navigate to /initiatives/:id?gate= — reusing
// DeliveryCycleDetailComponent's auto-expand gate sub-panel (D-345), not a fork.
...
import { PendingApprovalItem, GateName } from '../../core/types/database';
...
@Component({
  ...
  template: `
    ...
    <!-- Filter panel (S-011 commit / S-013 drill-in) -->
    <div *ngIf="panelOpen" class="ga-panel">
      <div class="ga-panel-row">
        ...
        <div *ngIf="openRow === 'gate'" class="ga-panel-opts">
          <label *ngFor="let g of gateKeys" class="ga-opt">
            <input type="checkbox" [checked]="stagedGates.has(g)" (change)="toggleStaged(stagedGates, g)" />
            {{ gateDisplay(g) }}
          </label>
        </div>
      </div>
      <div class="ga-panel-row">
        ...
        <div *ngIf="openRow === 'division'" class="ga-panel-opts">
          <label *ngFor="let d of divisionOptions" class="ga-opt">
            <input type="checkbox" [checked]="stagedDivisions.has(d)" (change)="toggleStaged(stagedDivisions, d)" />
            {{ d }}
          </label>
      ...
      <div class="ga-panel-row">
        ...
        <div *ngIf="openRow === 'date'" class="ga-panel-opts">
          <label class="ga-opt">
            <input type="checkbox" [checked]="stagedDateActive" (change)="stagedDateActive = !stagedDateActive" />
            Last {{ days }} days only
          </label>
        </div>
      </div>
    ...
    <!-- Skeleton rows during load (D-346) -->
    ...
    <div *ngIf="!loading" class="ga-grid" role="table">
      ...
      <div class="ga-row" role="row" *ngFor="let item of view; trackBy: trackByItem">
        ...
        <!-- D-181: tappable Initiative chip → detail with gate auto-expanded -->
        ...
        <a class="ga-action-btn" [routerLink]="['/initiatives', item.delivery_cycle_id]"
           [queryParams]="{ gate: item.gate_name }">Approve</a>
      </div>

      <!-- Empty state -->
      <div *ngIf="view.length === 0" class="ga-empty">
        No gate approvals in the last {{ days }} days.
  ...
  styles: [`
    ...
    .ga-chip { display: inline-flex; align-items: center; gap: 4px; background: rgba(37,112,153,0.08); color:#257099;
    ...
    .ga-panel-rowhead { display:flex;justify-content:space-between;width:100%;background:none;border:none;cursor:pointer;
    ...
    .ga-grid-head, .ga-row { display:grid; grid-template-columns: 130px 1.4fr 90px 110px 110px 80px 90px; gap:8px; padding:12px 16px; align-items:center; }
    ...
    .ga-action-btn { display:inline-block; background: var(--triarq-color-primary,#257099); color:#fff; border-radius:5px;
                     padding:4px 12px; font-size:12px; text-decoration:none; text-align:center; }
...
export class GateApprovalsTabComponent implements OnChanges {
  ...
  // Staged (panel) state — S-011 commit model.
  ...
  ngOnChanges(): void {
    if (!this.restored) {
      this.restored = true;
      this.screenState.restore(SCREEN_KEYS.ACTIONS_GATE_APPROVALS).then(state => {
        if (state) {
  ...
  // ── Filter panel (S-011 / S-013) ──────────────────────────────────────────
  toggleRow(row: 'gate' | 'division' | 'date'): void {
    this.openRow = this.openRow === row ? null : row;
  }
  toggleStaged(set: Set<string>, value: string): void {
  ...
  clearAll(): void {
    ...
    // S-011: Clear all resets staged without querying; panel stays open.
  }

  // ── Active-chip dismissal (S-012: immediate re-query, no Apply) ────────────
  removeDate(): void { this.appliedDateActive = false; this.stagedDateActive = false; this.persist(); }
  removeGate(g: string): void {
    this.appliedGates = this.appliedGates.filter(x => x !== g);
    this.stagedGates.delete(g);
    this.persist();
  }
  removeDivision(d: string): void {
    this.appliedDivisions = this.appliedDivisions.filter(x => x !== d);
    this.stagedDivisions.delete(d);
    this.persist();
  }
  ...
  private persist(): void {
    this.screenState.save(
      SCREEN_KEYS.ACTIONS_GATE_APPROVALS,
