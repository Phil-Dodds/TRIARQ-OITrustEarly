...
// Email-OTP sign-in surface (D-354). Submits an email address; AuthService sends
// a 6-digit code; component navigates to /auth/verify-otp passing email as
// router state.
//
// D-351: "Keep me signed in" persistence flag is written to localStorage before
// signInWithOtp is called so the hybrid storage adapter routes the resulting
// session to the correct backend.
// D-302 (amended by D-354): a failure of the send step shows the generic
// "Something went wrong. Please try again." — no account-existence info leaked.
// Per-failure messaging on OTP entry is on /auth/verify-otp itself.
...
@Component({
    ...
    <div class="login-wrapper">
      ...
      <div class="login-left">
        ...
        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" novalidate>
          ...
          <!-- Remember me (default: checked) — D-351 -->
          <div class="below-email-row">
          ...
          <!-- Send error (D-302 amended — no account enumeration) -->
          <div *ngIf="state === 'error'" class="oi-error-block">
            <p class="oi-error-primary">{{ errorMessage }}</p>
          </div>

          <button type="submit" class="signin-button" [disabled]="state === 'sending'">
            {{ state === 'sending' ? 'Sending code…' : 'Sign In' }}
          </button>
  ...
  styles: [`
    ...
    .below-email-row { display: flex; align-items: center; margin-bottom: 1rem; }
    ...
    .oi-error-primary { font-size: 13px; font-weight: 600; color: #c0392b; margin: 0; }
...
export class LoginComponent implements OnInit {
  ...
  async onSubmit(): Promise<void> {
    ...
    const { email, rememberMe } = this.loginForm.value as {
      email: string; rememberMe: boolean;
    };

    const result = await this.auth.signInWithOtp(email, rememberMe);
    ...
    await this.router.navigate(['/auth/verify-otp'], {
      state: { email: email.trim().toLowerCase(), rememberMe }
    });
  }
}
