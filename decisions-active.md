...
**D-172** — Delivery Cycle Dashboard Column Set. Eleven columns in order: Division, Cycle Title, Outcome Statement, Stage Track (Condensed), Headline, Assigned DS, Assigned CB, Pilot Start Date, Production Release Date, Tier, Delivery Workstream. Division, Cycle Title, Stage Track (Condensed), and Tier are never hidden regardless of viewport or user role. Remaining columns may be hidden on narrow viewports. | Source: Claude Chat | April 2026 |
| impl_status: specced |

**D-173** — assigned_ds_user_id as Sole DS Field. assigned_ds_user_id is the single field representing the Domain Strategist on a Delivery Cycle. cycle_owner_user_id is dropped — it referred to the same person and was redundant. Migration 025 copies cycle_owner_user_id → assigned_ds_user_id WHERE assigned_ds IS NULL, then drops the column. | Source: Claude Chat | April 2026 |
| impl_status: built |

**D-174** — DS and CB Nullable at Creation. assigned_ds_user_id and assigned_cb_user_id are nullable at Delivery Cycle creation. Gate enforcement via business rules MCP only — never a hard DB NOT NULL constraint. MCP blocks Brief Review gate if DS is null; blocks Go to Build gate if CB is null. See CC-006 for full enforcement detail. | Source: Claude Chat | April 2026 |
| impl_status: built |

**D-188** — Delivery Module Sub-Routes. [Renumbered from D-172 — collision with Claude Chat D-172 Dashboard Column Set. Renumbered 2026-04-05.] The Delivery feature module uses the following sub-routes under `/delivery`: (1) `/delivery` → DeliveryHubComponent (hub, no data); (2) `/delivery/workstreams` → WorkstreamSummaryComponent; (3) `/delivery/divisions` → DivisionSummaryComponent; (4) `/delivery/gates` → GatesSummaryComponent; (5) `/delivery/cycles` → DeliveryCycleDashboardComponent (the "big list" view, moved from the former root `/delivery` path to this sub-path); (6) `/delivery/:cycle_id` → DeliveryCycleDetailComponent. Named sub-routes are declared before the `:cycle_id` param route to avoid routing conflicts. The sidebar still links to `/delivery` (the hub). | Source: Claude Code | April 2026 |
| impl_status: built |

**D-189** — Next Gate Derived from Current Lifecycle Stage. [Renumbered from D-173 — collision with Claude Chat D-173 DS Field. Renumbered 2026-04-05.] The "next gate" a cycle must clear is determined by its current lifecycle stage, not by querying gate_records. Mapping: BRIEF → brief_review; DESIGN, SPEC → go_to_build; BUILD, VALIDATE → go_to_deploy; PILOT, UAT → go_to_release; RELEASE, OUTCOME → close_review; COMPLETE, CANCELLED, ON_HOLD → null (no next gate). This mapping is defined in `lifecycle.js` as `NEXT_GATE_BY_STAGE` and shared by the `get_delivery_summary` MCP tool and the Angular dashboard component. | Source: Claude Code | April 2026 |
| impl_status: built |

**D-190** — WIP Categories and Limit. [Renumbered from D-174 — collision with Claude Chat D-174 DS/CB Nullable. Renumbered 2026-04-05.] Work-in-progress (WIP) is measured per workstream in three categories: Prep (stages BRIEF, DESIGN, SPEC), Build (stages BUILD, VALIDATE), and Outcome (stages PILOT, UAT, RELEASE, OUTCOME). WIP limit per category per workstream is 4. COMPLETE and CANCELLED cycles excluded. ON_HOLD included (occupies a slot). WIP constants defined in `lifecycle.js` as `WIP_CATEGORY_BY_STAGE` and `WIP_LIMIT`. | Source: Claude Code | April 2026 |
| impl_status: built |
...
### Build C Decisions — Contract 5 (April 2026)

**D-272** — Workstream Filter: Peer Options Model with Terminal Selection Exception. The Workstream filter in the All Delivery Cycles filter panel uses a three-option peer model: (1) Normal List — scope activator showing workstreams homed in the user's own Divisions; (2) Bigger List — scope activator showing all loaded workstreams sorted by Division hierarchy; (3) No Workstream Assigned — terminal selection. Normal and Bigger List selections are staged and require Apply per S-011. No Workstream Assigned applies immediately on selection — named exception to S-011. Workstreams sorted: active first, then by home_division_name, then workstream_name. Division short name shown per workstream row. wsScope state variable ('' | 'normal' | 'bigger' | 'none_terminal') tracks filter state; '' = no filter active per D-278. Amended 2026-04-12 by Contract 5 to introduce peer options model, replacing the Normal/Bigger toggle from Contract 4. Implementation in D-285. Connects to: S-011, S-013, D-278, D-279, D-285. | Source: Claude Code | April 2026 |
| impl_status: built |

**D-277** — Assigned Person Filter: Peer Options with Immediate Terminal Selections. The Assigned Person filter in the All Delivery Cycles filter panel provides five peer options: (1) Normal List — scope activator showing persons from cycles in the user's own Divisions; (2) Bigger List — scope activator showing all persons across loaded cycles; (3) Me — terminal, filters to cycles where the current user is assigned DS or CB; (4) Unassigned DS — terminal, filters to cycles with no assigned DS; (5) Unassigned CB — terminal, filters to cycles with no assigned CB. Me, Unassigned DS, and Unassigned CB apply immediately on selection — named exception to S-011 per the pattern established in D-272. Normal and Bigger list selections are staged and require Apply. Person list sourced from loaded cycle DS and CB assignment data — no server-side person lookup tool required at this stage. Server-side person lookup deferred to Build B. Implementation in D-286. Connects to: S-011, D-272, D-278, D-286. | Source: Claude Chat | April 2026 |
| impl_status: built |

**D-278** — No-Filter Default: Absence of Selection Equals No Filter Applied. In all filter rows in the All Delivery Cycles filter panel, absence of selection means no filter is applied for that dimension — not "All." No radio button, label, or option labelled "All Stages," "All Tiers," "All," or "Anyone" is shown. When a filter row has no value selected (empty string state), all cycles pass through that dimension. This removes the named "All" options that previously appeared at the top of Stage, Gate Status, Tier, and Assigned Person filter rows. The Division filter's "My Divisions (default)" label with empty value is not removed — it is a scope default, not a named "All." Connects to: S-011, D-272, D-277, D-285, D-286. | Source: Claude Chat | April 2026 |
| impl_status: specced |

**D-279** — Workstream Short Name Display Standard: display_name_short with workstream_name Fallback. All Workstream display surfaces — filter rows, Identity Zone chips, workstream cells — display using display_name_short when populated, falling back to workstream_name when null per D-203. Workstream chip label formula: [home_division_short ?? home_division_name] · [display_name_short ?? workstream_name]. This is the operative display rule for all Workstream references across the application following Migration 027. Existing records receive null display_name_short at migration time; fallback ensures no visual regression. Admins populate display_name_short (max 20 chars recommended per D-203) via Workstream admin interface. Prerequisite: D-283 (Migration 027). Connects to: D-203, D-283, D-285. | Source: Claude Chat | April 2026 |
| impl_status: built |

**D-280** — Contract 5 Session Classification and Regression Protection Checkpoint. Contract 5 is classified MODIFICATION for all three surfaces: All Delivery Cycles grid, Delivery Cycle Detail panel, and All Delivery Cycles filter panel. Regression protection checkpoint required at plan-mode before any code is written: confirmed-clean Contract 4 items are declared and must not regress. Confirmed-clean Contract 4 items: Outcome Statement zone plain italic text no inline Add/Edit link (D-276); Gate rows target date editable, actual date editable, status dropdown (D-275); Filter panel slide-in Apply/Clear All/X (S-010, S-011); Filter chips Name:Value format dismissible (S-012); Filter accordion one row open (S-013); Identity Zone DS chip, CB chip, Tier, Workstream, Jira Epic; Cycle Artifacts zone; Jira Sync zone; Event Log zone. Any change to confirmed-clean items requires a new decision number. Connects to: D-239, D-275, D-276, S-010 through S-014. | Source: Claude Chat | April 2026 |
| impl_status: built |

**D-281** — Tier Badge Pill Restored to Grid Cycle Name Cell. The Tier badge pill displayed below the cycle title in the All Delivery Cycles grid row is restored. Pill renders directly below cycle title text with 4px gap, left-aligned. Spec: Tier 1 background #E3F2FD text #1565C0; Tier 2 background #E0F2F1 text #00695C; Tier 3 background #FFF3E0 text #E65100; border-radius 4px; padding 3px 8px; Roboto Medium 12px. Partial amendment to D-264: only the badge pill below the cycle title is reintroduced. The Tier avatar dot (D-197, superseded by D-264) remains gone. All other operative text in D-264 remains in force. Supersedes: D-264 badge pill removal only. Connects to: D-264, D-172, Visual Layout Standards Section 1.7. | Source: Claude Code | 2026-04-12 |
| impl_status: built |

**D-282** — Gate Status Initialization: not_started Seed on New Cycles. New delivery cycles seed gate_status = 'not_started' in gate_records at creation. gateDetailStatus() handles 'not_started' as "Not Started" displayed in grey. Gate status "Under Review — awaiting decision" must not appear until an explicit submission action has been recorded in gate_records. Prior behavior derived status from cycle stage rather than gate_records data — this decision eliminates that inference. Connects to: D-273, D-275, build-c-spec.md gate rows. | Source: Claude Code | 2026-04-12 |
| impl_status: built |

**D-283** — Migration 027: display_name_short Column on delivery_workstreams. A nullable display_name_short text column is added to the delivery_workstreams table via migration 027 (027_add_display_name_short_to_workstreams.sql). Existing records receive NULL, triggering the D-203 fallback to workstream_name on all display surfaces. The column is admin-populable without further code change. list_delivery_workstreams.js selects and passes through display_name_short. DeliveryWorkstream interface in database.ts carries display_name_short?: string | null. All workstream display surfaces use display_name_short ?? workstream_name per D-203. Prerequisite for D-272, D-279 correct display. Connects to: D-203, D-279, ARCH-29. | Source: Claude Code | 2026-04-12 |
| impl_status: built |

**D-284** — [PLACEHOLDER — text deferred pending UAT] Zone Explanatory Text Styling. Zone-level explanatory text in the Delivery Cycle Detail View rendered at font-size: 11px; font-style: italic; color: #5A5A5A. Applied to Gate Record, Artifacts, Jira Sync, and Event Log zone explanatory paragraphs. Phil to confirm at UAT whether this becomes Active Standard S-015. Number claimed 2026-04-12. Full text drafted after UAT confirmation. Source: Contract 5 Block 2.5. | Source: Claude Code | 2026-04-12 |
| impl_status: specced |

**D-285** — Workstream Filter: wsScope State Variable and Peer Options Model. The workstream filter component state replaces wsListSize: 'normal' | 'bigger' with wsScope: '' | 'normal' | 'bigger' | 'none_terminal'. '' = no scope selected, no list shown, no filter active (D-278). 'normal' and 'bigger' are scope activators — each renders a scoped workstream list below; list selection is staged and requires Apply per S-011. 'none_terminal' is the radio value for No Workstream Assigned — calls applyFilterPanel() immediately on change; named exception to S-011 per D-272 amended. Normal List scope: workstreamsScopedNormal getter (workstreams where home_division_id is in userDivisions). Bigger List scope: workstreamsSortedByDivision (full loaded list). Workstream chip label uses workstreamChipLabel() helper returning [home_division_name] · [display_name_short ?? workstream_name] per D-279. clearStagedFilters() resets wsScope = ''. openFilterPanel() restores wsScope from applied filter on panel open. Connects to: D-272, D-278, D-279, D-203, S-011, S-013. | Source: Claude Code | 2026-04-12 |
| impl_status: built |
