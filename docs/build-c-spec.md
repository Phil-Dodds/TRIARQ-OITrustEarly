...
29. **Maintenance mode implemented and verified:** system_config table seeded, set_maintenance_mode and get_maintenance_mode tools operational in division-mcp, Angular bootstrap interception confirmed (maintenance screen renders when flag is true, normal routing resumes when false), Claude Code deployment sequence documented and tested
    — **MET 2026-08-02 (Contract 42).** Both tools registered in `mcp/division-mcp/src/index.js` (router map + `/tools` discovery) and admin-gated per D-635. Bootstrap interception is the `APP_INITIALIZER` in `AppModule` calling `MaintenanceModeService`, which reads `system_config` directly under the reinstated Arch-1 exception before any route resolves and, when the flag is on, empties the route table so no guard runs and no auth is attempted. `MaintenanceScreenComponent` renders as the entire application. 8 tests in `mcp/division-mcp/tests/contract42-maintenance-mode.test.js`. `system_config` confirmed seeded — one row, verified against the live schema. Per **D-636** there is deliberately **no admin UI** for the toggle: maintenance mode suppresses routing and attempts no auth, so an in-app control could enable the state and could never clear it. The operator in §9 is Code or Phil at a terminal. Recorded as an explicit D-310 exception. Awaiting Phil's UAT.
